...
function PDFDemo() {
  ...
  const clauseFound = verifyExample(hasClause ? 'Article 04 : Protection des données' : 'Article 04 : Prestations');
  const phase = frame % 3;
  const result = phase === 0 ? 'Contrat reçu' : phase === 1 ? 'Recherche de la clause…' : clauseFound ? 'Contrôle satisfait' : 'Contrôle à reprendre';
  return <div className="pdf-demo" ref={panel}>
    <div className="visual-label"><span>01 / CONTRÔLE D’UNE CLAUSE</span><span>EXEMPLE FICTIF</span></div>
    <div className="pdf-steps" aria-label="Étapes du contrôle"><span className={phase === 0 ? 'current' : ''}>01 Recevoir</span><span className={phase === 1 ? 'current' : ''}>02 Vérifier</span><span className={phase === 2 ? 'current' : ''}>03 Signaler</span></div>
    <div className={`document-example contract-example ${phase === 1 ? 'scanning' : ''}`}>
      <span className="mono">CONTRAT FOURNISSEUR · DÉMONSTRATION</span>
      <h4>Accord de prestation</h4>
      <div className="document-lines" aria-hidden="true"><i/><i/><i/></div>
      <p className={`contract-clause ${clauseFound ? 'present' : phase === 2 ? 'missing' : ''}`}><span>ARTICLE 04</span><strong>{clauseFound ? 'Protection des données' : 'Prestations et livraison'}</strong></p>
      {phase === 1 && <span className="pdf-scan-line" aria-hidden="true"/>}
    </div>
    <div className={`check-result pdf-result ${phase === 2 && !clauseFound ? 'failed' : ''}`} role="status"><span>{phase === 2 ? clauseFound ? '✓' : '!' : phase === 1 ? '⌕' : '↓'}</span><div><strong>{result}</strong><small>{phase === 2 ? clauseFound ? 'La clause attendue est repérée.' : 'Clause manquante : revue humaine requise.' : 'Règle : repérer la clause de protection des données.'}</small></div></div>
    <div className="pdf-controls"><button type="button" aria-pressed={!hasClause} onClick={() => { setPlaying(false); setFrame(2); }}>Clause absente</button><button type="button" aria-pressed={hasClause} onClick={() => { setPlaying(false); setFrame(5); }}>Clause ajoutée</button><button type="button" onClick={() => { if (playing) setPlaying(false); else { setFrame(0); setPlaying(true); } }} disabled={reduced}>{playing && !reduced ? 'Ⅱ Pause' : '▶ Revoir'}</button></div>
    <p className="pdf-benefit">Un contrôle répétitif automatisé peut signaler un oubli avant validation et libérer du temps pour la revue humaine.</p>
  </div>;
}
